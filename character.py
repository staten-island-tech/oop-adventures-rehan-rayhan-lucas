class Character:
    ...
    def question(self, inventory):
        if "Fingerprint Kit" in inventory.items and self.role == "maid":
            return f"{self.name} nervously says: 'Okay, fine! I did it. I left the handkerchief.'"
        elif self.role == "maid":
            return f"{self.name} says: 'I was cleaning. You can't prove anything else.'"
        return f"{self.name} says: '{self.alibi}'"
